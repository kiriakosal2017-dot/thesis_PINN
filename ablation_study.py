...
# Each config: model flags + which meta columns to keep out of the features.
# (defaults preserve the full PI-NODE; only the named knob changes per row)
CONFIGS = {
...
def run_config(name, cfg, epochs, results_dir):
    ...
    # 'with_acceleration' needs a different feature split, so reload sequences for it.
