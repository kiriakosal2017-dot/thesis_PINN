...
> ⚠️ **Pending regeneration.** The numbers below were produced before the methodology
> revision (train-only imputation, `dt`/`acceleration` removed from inputs, shuffled
> HYBRID training, corrected HYBRID boundary condition, seeded runs). They must be
> regenerated by re-running the pipeline before they are valid again.
